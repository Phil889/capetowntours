...
import { POPULAR_TOUR_SLUGS } from "@/lib/tour-data/tour-constants";
...
// Static revalidation configuration
...
// Generate metadata for SEO with locale support
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  try {
    ...
    console.error('[Localized Tour Detail Error - generateMetadata]:', error);
    return TourMetadataGenerator.generate(null, '');
  }
}

// Pre-generate static pages for popular tours in all locales
...
export default async function LocalizedTourDetailPage({ params }: PageProps) {
  const { locale, slug } = await params;
  ...
  // Fetch tour data with locale support
  let tour;
  try {
    tour = await TourRepository.getBySlugAndLocale(slug, locale);
  } catch (error) {
    console.error('[Localized Tour Detail Error - getTour]:', error);
    notFound();
  }
  ...
  // Load translations server-side for consistent SSR
  ...
  const tourUrl = `${baseUrl}${localePath}/tours/${tour.slug}`;
      ...
      >
        <TourPageTemplate 
          ...
          translations={{
            ...
            booking: {
              per_person: allTranslations['booking.per_person'] || bookingTranslations['per_person'],
              best_price_guarantee: allTranslations['booking.best_price_guarantee'] || bookingTranslations['best_price_guarantee'],
              people_viewing_now: allTranslations['booking.people_viewing_now'] || bookingTranslations['people_viewing_now'],
              booked_today: allTranslations['booking.booked_today'] || bookingTranslations['booked_today'],
              max_group_size: allTranslations['booking.max_group_size'] || bookingTranslations['max_group_size'],
              top_rated: allTranslations['booking.top_rated'] || bookingTranslations['top_rated'],
              verified_operator: allTranslations['booking.verified_operator'] || bookingTranslations['verified_operator'],
              operator: allTranslations['booking.operator'] || bookingTranslations['operator'],
              rating: allTranslations['booking.rating'] || bookingTranslations['rating'],
              chat_with_us: allTranslations['booking.chat_with_us'] || bookingTranslations['chat_with_us'],
              call_us: allTranslations['booking.call_us'] || bookingTranslations['call_us'],
              free_cancellation_24h: allTranslations['booking.free_cancellation_24h'] || bookingTranslations['free_cancellation_24h'],
              recent_guest_review: allTranslations['booking.recent_guest_review'] || bookingTranslations['recent_guest_review'],
              from: allTranslations['booking.from'] || bookingTranslations['from'],
              check_availability: allTranslations['booking.check_availability'] || bookingTranslations['check_availability'],
              max: allTranslations['booking.max'] || bookingTranslations['max'],
              book_your_tour: allTranslations['booking.book_your_tour'] || bookingTranslations['book_your_tour']
            },
            tour_detail: {
        ...
        >
          ...
          />
          <TourReviews tourSlug={tour.slug} locale={locale} />
        </TourPageTemplate>
      </TranslationProvider>
    </>
  );
